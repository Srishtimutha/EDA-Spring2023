...
epa_metadata <- data.frame(c("Source", "Date", "Filename"),
                       c("EPA Air Quality SYstem (AQS)", "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv"))
colnames(epa_metadata) <- c("Attribute", "Value")
kable(epa_metadata,
      align = "rl")
```
...
kable(summary_PM25, caption = "Mean Particulates", col.names = c("County", "$\\mu g/m^3$"))
...
```{r echo=FALSE, fig.align='left', fig.height=2.5, fig.width = 6, fig.cap = "Ozone levels by month across 2018"}
epa_data %>%
  filter(Year == 2018) %>%
  drop_na() %>%
  #group_by(Month) %>%
  ggplot(mapping = aes(x = month(Month, label = TRUE),
                       y = Ozone)) +
  geom_boxplot() +
  theme_minimal() +
  labs(x = "Month",
       y = "Ozone Concentration (ug/m^3)")
```
...
### Git Repository
SrishtiMutha Git Repository: https://github.com/Srishtimutha/EDA-Spring2023.git
